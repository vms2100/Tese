...
def insert_libraries(n=500000):
    for _ in range(n):
        cursor.execute("""
            ...
            VALUES (%s, %s, %s)
        """, (fake.text(max_nb_chars=100), fake.text(max_nb_chars=250), fake.text(max_nb_chars=150)))
...
def insert_authors(n=1000000):
    for _ in range(n):
        cursor.execute("""
            ...
            VALUES (%s, %s, %s)
        """, (fake.text(max_nb_chars=100), random.randint(25, 85), fake.text(max_nb_chars=200)))
...
def insert_books(n=1500000):
    for _ in range(n):
        cursor.execute("""
            ...
            VALUES (%s, %s, %s)
        """, (fake.isbn13(), fake.text(max_nb_chars=50), fake.text(max_nb_chars=150)))
...
def insert_clients(n=100000):
    for _ in range(n):
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (fake.text(max_nb_chars=100), fake.phone_number(), fake.text(max_nb_chars=200), fake.text(max_nb_chars=100)))

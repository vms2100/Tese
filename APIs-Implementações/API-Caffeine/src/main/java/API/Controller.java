...
public class Controller {
    ...
    public Controller(BookRepository bookRepository, AuthorRepository authorRepository, BookAuthorRepository bookAuthorRepository, LibraryRepository libraryRepository, ReservationRepository reservationRepository, ClientRepository clientRepository,StockRepository stockRepository) {
    ...
    public ResponseEntity<byte[]> getAuthors(@PathVariable String format, @RequestParam(required = false) Integer ID_Authors, @RequestParam(required = false) String Name, @RequestParam(required = false) Integer age, @RequestParam(required = false) String Bibliography, @RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        List<Author> authors = CacheManager.AuthorSearchCache.getIfPresent(key);
        ...
        if (authors == null) {
            ...
            CacheManager.AuthorSearchCache.put(key,authors);
        }
    ...
    public ResponseEntity<byte[]> getClients(@PathVariable String format, @RequestParam(required = false) Integer ID_Client, @RequestParam(required = false) String Name, @RequestParam(required = false) String Phone, @RequestParam(required = false) String Email, @RequestParam(required = false) String Adress, @RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        List<Client> clients = CacheManager.ClientSearchCache.getIfPresent(Key);
        ...
        if (clients == null){
            ...
            CacheManager.ClientSearchCache.put(Key,clients);
        }
        ...
        for (Client client : clients) {
            CacheManager.ClientCache.put(client.getId(),client);
    ...
    public ResponseEntity<byte[]> getReservations(@PathVariable String format, @RequestParam(required = false) Integer ID_Reservations, @RequestParam(required = false) Integer ID_Library, @RequestParam(required = false) Integer ID_Book, @RequestParam(required = false) Integer ID_Clients, @RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        String key = cacheKeyReservation(ID_Reservations, ID_Library, ID_Book, ID_Clients);

        List<Reservation> reservations= CacheManager.ReservationSearchCache.getIfPresent(key);

        if (reservations == null) {
            long dbTime2 = System.nanoTime();
            reservations = reservationRepository.getReservationParamether(ID_Reservations, ID_Library, ID_Book, ID_Clients);
            dbTime += duration(dbTime2);
            CacheManager.ReservationSearchCache.put(key, reservations);
        }
    ...
    public ResponseEntity<byte[]> getBooks(@PathVariable String format,@RequestParam(required = false) Integer ID_Book,@RequestParam(required = false) String ISBN, @RequestParam(required = false) String Title, @RequestParam(required = false) String Sinopse, @RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        List<Book> books = CacheManager.BookSearchCache.getIfPresent(Key);

        if (books==null) {
            long startDb1 = System.nanoTime();
            books = bookRepository.getBooksbyParamether(ID_Book, ISBN, Title, Sinopse);
            dbTime += duration(startDb1);
            CacheManager.BookSearchCache.put(Key, books);
        }
        ...
        long parseTime=0;
        for (Book book : books){
            CacheManager.BookCache.get(book.getid(), k -> book);
            String key = format + book.getid();

            String s =CacheManager.BookCacheS.getIfPresent(key);
            if (s == null){
                ...
                CacheManager.BookCacheS.put(key,resultQ);
            }else{
                response.append(s);
            }
    ...
    public ResponseEntity<byte[]> getBooksByAuthorName(@PathVariable String format,@PathVariable("Author_Name") String Author_Name,@RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        for (Author author : authors){
            CacheManager.AuthorCache.get(author.getId(), k -> author);
            ...
            for (Integer bookID : bookIDs){
                books.add(CacheManager.BookCache.get(bookID, null));
            ...
            for (Book book : books){
                ...
                if (s == null){
                    ...
                    CacheManager.BookCacheS.put(key,s);
                }
                response.append(s);
            }
            parseTime+=duration(parseTime2);
    ...
    public ResponseEntity<byte[]> getBooksByAuthorID(@PathVariable String format,@PathVariable("Author_ID") Integer Author_ID,@RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        for (Author author : authors){
            ...
            for (Integer bookID : booksIds){
                Book book = CacheManager.BookCache.getIfPresent(bookID);
                if (book!=null){
                    books.add(book);
    ...
    public ResponseEntity<byte[]> getAuthorsByBookName(@PathVariable String format,@PathVariable("Book_Name") String Book_Name,@RequestParam(defaultValue = "false") boolean download) throws JsonProcessingException {
        ...
        for (Book book : books){
            ...
            if (s == null){
                ...
                response.append(s2);
                CacheManager.BookCacheS.put(key,s2);
                parseTime+=duration(parseTime2);
            }else{
                response.append(s);
            }
    ...
    public ResponseEntity<?> getReservationInfoByReservationID(@PathVariable String format, @PathVariable Integer ID_Reservation) throws JsonProcessingException {
        ...
        Reservation reservation =CacheManager.ReservationCache.getIfPresent(ID_Reservation);
        ...
        if (reservation==null) {
            ...
            if (reservations.isEmpty()) {
                ...
                CacheManager.ReservationCache.put(ID_Reservation,r1);
        ...
        Client client = CacheManager.ClientCache.getIfPresent(reservation.getIdClients());
        Book book = CacheManager.BookCache.getIfPresent(reservation.getIdBook());

        if (client == null){
            long dbtime2=System.nanoTime();
            clients = clientRepository.getClientsParamether(reservation.getIdClients(), null, null, null, null);
            dbTime += duration(dbtime2);
            CacheManager.ClientCache.put(reservation.getIdClients(),clients.getFirst());
        }else{
            clients.add(client);
        }
        ...
        if (book == null){
            ...
            dbTime += duration(dbtime2);
            CacheManager.BookCache.put(reservation.getIdClients(),books.getFirst());
        }else{
            books.add(book);
        }
        ...
        Author author = CacheManager.AuthorCache.getIfPresent(authorId);
        if (author == null){
            dbtime2=System.nanoTime();
            List<Integer> authorIds = Collections.singletonList(authorId);
            authors = authorRepository.getAuthorsByIDs(authorIds);
            if (!authors.isEmpty()) {
                CacheManager.AuthorCache.put(authorId, authors.getFirst());
            }
            dbTime += duration(dbtime2);
        }else{
            authors.add(author);
    ...
    public ResponseEntity<?> getReservationInfoByClientName(@PathVariable String format, @PathVariable String ClientName) throws JsonProcessingException {
        ...
        long dbtime2=System.nanoTime();
        for (Client client : clients) {
            CacheManager.ClientCache.get(client.getId(), k -> client);
        ...
        for (Reservation reservation : reservations) {
            ...
            Client client1 = CacheManager.ClientCache.getIfPresent(reservation.getIdClients());
            Book book = CacheManager.BookCache.getIfPresent(reservation.getIdBook());
            if (book == null){
       ...
       dbTime += duration(dbtime2);
        ...
        for (Reservation reservation : reservations) {
            ...
            if (client1 == null){
                ...
                dbTime += duration(dbtime3);
                CacheManager.ClientCache.put(client.getFirst().getId(), client.getFirst());
            }else{
                client.add(client1);
            }
    ...
    public ResponseEntity<?> getStockInfoByLibrary(@PathVariable String format, @PathVariable String Library) throws JsonProcessingException {
        ...
        res = CacheManager.StockSearch.getIfPresent(cacheKey);
        if (res != null) {
                if (res.isEmpty()) {
        ...
        CacheManager.StockSearch.put(cacheKey,res);
    ...
    private String cacheKeyReservation(Integer ID_Reservation,Integer ID_Library,Integer ID_Book, Integer ID_Clients){
        StringBuilder sb = new StringBuilder();

        if (ID_Reservation != null) {
            sb.append("ID_Reservation=").append(ID_Book);
        }
        if (ID_Library != null) {
            sb.append("ID_Library=").append(ID_Library);
        }
        if (ID_Book != null) {
            sb.append("ID_Book=").append(ID_Book);
        }
        if (ID_Clients != null) {
            sb.append("ID_Clients=").append(ID_Clients);
        }
        return sb.toString().trim();
    }
